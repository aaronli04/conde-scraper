...
import pandas as pd
from helpers import extract_soup_from_response, extract_hotel_name, extract_primary_hotel_link, scrape_hotel_details, extract_awards_list, extract_description
...
def scrape_conde():
    ...
    url = 'https://www.cntraveler.com/gallery/best-hotels-in-london'

    response = requests.get(url)
    soup = extract_soup_from_response(response)

    data = {'Hotel Name': [], 'Awards': [], 'Price Point': [], 'Description (List)': [
    ], 'Description (Hotel)': [], 'Address': [], 'Conde Hotel URL': [], 'Hotel Website URL': []}
    ...
    for caption in hotel_captions:
        ...
        if detail_element:
            ...
            external_link, paragraphs, address = scrape_hotel_details(
                primary_hotel_link)
